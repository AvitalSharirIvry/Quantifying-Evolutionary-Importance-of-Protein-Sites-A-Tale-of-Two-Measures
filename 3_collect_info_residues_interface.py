...
def read_funct_res(list_file):
    ...
    for line in list_file:
        yeast,pdb,chain=line.split()[0],line.split()[1],line.split()[2][0]
        if len(line.split())>3 :
...
# for each protein - yeast with model
    ...
    for line in chosen_file:
        ...
        if pdb+chain in consurfDB_file_lines and pdb+chain in Cax_of_pdb and yeast+pdb+chain in functional_ress and functional_ress[yeast+pdb+chain]!=[] and yeast+pdb+chain not in yeast_pdb_chain:
...
#Pearson correlation (conservation,dist) for each residue
    ...
    df=df.transpose()
    df.to_csv('figure2_interfaces.txt',sep=',')
    return
...
run('chosen_structural_homologs','consurfDB_file','list_interfaces')
