...
def read_funct_res(list_file):
    ...
    for line in list_file:
        ...
        if len(line.split())>3 and (line.split()[3]).isalpha()==True:
            lig=line.split()[3]
            funct_ress=line.split()[4:]
        elif len(line.split())>3 and (line.split()[3]).isalpha()==False:
...
# overall dataset average pearson vs binned ranks 
    ...
    df=df.transpose()
    df.to_csv('figure2_exposed_nonfunct.txt',sep=',')
...
run('chosen_structural_homologs','consurfDB_file','list_res_exposed_nonfunct')
